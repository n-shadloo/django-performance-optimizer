...
**Django ships the API, not a production worker** — real execution needs an external backend/worker. Task args and return values must be **JSON-serializable** (pass IDs, not model instances). `TaskResult` states: `READY` → `RUNNING` → `SUCCESSFUL` / `FAILED`. Use `queue_name` and `priority`; `takes_context=True` gives the task access to attempt count.
